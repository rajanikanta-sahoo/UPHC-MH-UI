...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (userDetails != null) {
      ...
      this.stateCode = userDetails.areaModel.areaCode;
  ...
  isStateCg() {
    let userDetails = JSON.parse(localStorage.getItem('access_token'))
    let authorities: string[] = userDetails.roles;
    if (authorities.includes('Admin') || authorities.includes('National')) {
      return true;
    } else {
      if (userDetails.areaModel.areaCode.substring(0, 6) == 'IND022')
        return true;
      else
        return false;
    }
