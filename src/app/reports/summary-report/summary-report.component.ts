...
import { DevinfoAreaModel } from 'src/app/models/DevinfoAreaModel';
declare var $:any; 
...
export class SummaryReportComponent implements OnInit {
  ...
  selectedState: DevinfoAreaModel = new DevinfoAreaModel();

  constructor(private summaryReportService:SummaryReportService) { }

  ngOnInit() {

    this.summaryReportService.getAllStates().then((data) =>{
  ...
  this.selectedProgram = new SectorModel();;
  ...
  this.selectedState = state;
    this.getSectorAndProgram();
  }

  getSectorAndProgram(){
    this.summaryReportService.getAllTimePeriods(this.selectedState.areaNid).then((data)=> {
      ...
      this.summaryReportService.getAllSectors().then((data) =>{
        ...
        this.allFacilitiesP =this.convert(this.allSectors).children;
      }).catch(error=>
        {})


        this.summaryReportService.getAllSectorPrograms(this.selectedState.areaId).then((data) =>{
     ...
     getRawData(row){
      ...
      //  ,this.selectedProgram.iC_NId,this.selectedSection.iC_NId
        this.summaryReportService.getRawData(row.rowId, this.selectedFacility.iC_NId, this.selectedSectionSubsection.iC_NId, this.selectedTimepriod.timePeriod_Nid,this.selectedProgram.iC_NId,this.selectedSection.iC_NId).then((data)=>{
          ...
          let index:number=0;
          let set = new Set();
          // for(let j=1;j<this.divisionalTableData.length;j++){
          //   if(Object.keys(this.divisionalTableData[j-1]).length < Object.keys(this.divisionalTableData[j]).length){
          //     index = j;
          //   }
          // }
          ...
          // this.divisionalTableColumns = Object.keys(this.divisionalTableData[index]);
          this.divisionalTableColumns = Array.from(set);
