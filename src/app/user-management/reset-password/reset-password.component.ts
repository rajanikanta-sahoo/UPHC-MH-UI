...
export class ResetPasswordComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    .subscribe((e: any) => {
        if(this.router.url =="/reset-password" &&e.url != '/edit-user' ){
          ...
          // this.userManagementService.formFieldsAll ={};
        }
    });
 ...
 getUsers(obj){
   this.p = 1;
   this.userManagementService.resetPasswordDetails = obj;
   this.userManagementService.resetPasswordDetails.selectedRoleId=obj.roleName;
    this.userManagementService.getUsersByRoleId(obj.id).subscribe(res => {
...
enableUserDetails(id){
  this.http.get(Constants.HOME_URL + 'enableUser?userId='+id).subscribe((data)=>{
    ...
    this.getUsers(this.userManagementService.resetPasswordDetails);
    // $("#disable").show; 
  }, err=>{      
  }); 
}
...
getRole(opt){
  this.userManagementService.resetPasswordDetails.selectedRoleId=opt.roleName;
}
getWidth(id) {
  return $("#" + id).width();
}
